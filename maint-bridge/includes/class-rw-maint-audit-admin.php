...
class RW_Maint_Audit_Admin {
	const MENU_SLUG = 'rw-maint-audit';
	const PER_PAGE  = 50;
	...
	public static function render() {
		...
		echo '<div class="wrap">';
		echo '<h1>ReactWoo Maintenance Audit Log</h1>';
	...
	private static function render_table( array $logs ) {
		...
		echo '<thead><tr>';
		echo '<th>ID</th><th>Event</th><th>User</th><th>Subscription</th><th>Portal Site</th><th>Action</th><th>Result</th><th>Message</th><th>Created</th>';
		echo '</tr></thead><tbody>';
	...
	private static function query_logs( array $filters, $limit, $offset ) {
		...
		if ( '' !== $filters['portal_site_id'] ) {
			$where[]  = 'portal_site_id = %d';
			$params[] = absint( $filters['portal_site_id'] );
		}
	...
	private static function export_csv() {
		...
		header( 'Content-Disposition: attachment; filename=rw-maint-audit.csv' );

		$output = fopen( 'php://output', 'w' );
		fputcsv( $output, array( 'id', 'event_type', 'user_id', 'subscription_id', 'portal_site_id', 'message', 'created_at' ) );
